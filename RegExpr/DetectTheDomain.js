/*
    Solution for the problem - https://www.hackerrank.com/challenges/detect-the-domain-name/problem
    //TODO: Can come up with better solusion
*/
...
`;
    ...
    let re = new RegExp(`([^\/]http:\/\/|[^\/]https:\/\/)(?!www\\.|ww2\\.){0,1}((\\w+|[a-zA-Z0-9-]+)\\.)+(\\w+)`,'g')
    let re1 = new RegExp(`((http:\/\/|\\(http:\/\/|https:\/\/|\\(https:\/\/)(www\\.|ww2\\.){0,1}|[" '=_])`,'g');
    let set = new Set(input.match(re).map(value => value.replace(re1,'')));
